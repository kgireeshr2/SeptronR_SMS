...
async def review_leave_application(
    ...
    # Notify the staff member of the decision (in-app + email).
    try:
        from sqlalchemy import select as _select
        from app.models.staff import Staff as _Staff
        from app.services.notifications.notify import notify_event
        _uid = (await db.execute(_select(_Staff.user_id).where(_Staff.id == leave.staff_id))).scalar()
        if _uid:
            _decision = "approved" if approved else "rejected"
            notify_event(
                school_id, "leave_decision", audience="custom", user_ids=[str(_uid)],
                default_channels=["in_app", "email"],
                title=f"Leave {_decision.capitalize()}",
                body=f"Your leave application has been {_decision}."
                     + (f" Remarks: {review_data.remarks}" if review_data.remarks else ""),
                link="/leaves",
            )
    except Exception:
        pass
