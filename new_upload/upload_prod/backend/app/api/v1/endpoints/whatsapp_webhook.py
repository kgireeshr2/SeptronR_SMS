...
def _verify_signature(raw_body: bytes, signature_header: Optional[str]) -> bool:
    ...
    Returns True when no app secret is configured (verification disabled) so existing
    setups keep working; the caller logs a warning in that case.
    """
    app_secret = settings.META_APP_SECRET
    if not app_secret:
        return True  # not configured — caller warns
...
async def _resolve_school_for_phone_id(db: AsyncSession, phone_id: Optional[str]) -> Optional[str]:
    """Map the receiving WhatsApp Business phone_number_id → school_id via per-school Settings.

    Returns None for single-shared-number setups (no per-school mapping configured).
    """
    if not phone_id:
        return None
    try:
        row = (await db.execute(
            select(SchoolSetting.school_id).where(
                SchoolSetting.category == "communication",
                SchoolSetting.key == "whatsapp_phone_id",
                SchoolSetting.value == str(phone_id),
            ).limit(1)
        )).first()
...
async def _wa_send_config(db: AsyncSession, school_id: Optional[str]) -> tuple[str, str]:
    """Resolve (token, phone_id) for sending — per-school Settings override global env."""
    token, phone_id = WA_TOKEN, WA_PHONE_ID
    if school_id:
        try:
            kv = await SettingsRepository(db).get_as_dict(school_id, "communication")
            token = kv.get("whatsapp_token") or kv.get("whatsapp_api_key") or token
            phone_id = kv.get("whatsapp_phone_id") or phone_id
        except Exception:
            pass
...
async def _lookup_user_by_phone(
    phone: str, db: AsyncSession, school_id: Optional[str] = None
) -> Optional[User]:
    ...
    # Scope to the tenant that owns the receiving WhatsApp number, when known —
    # prevents a phone-number collision from crossing schools.
    if school_id:
...
async def _send_wa_message(
    to: str, payload: Dict[str, Any], token: Optional[str] = None, phone_id: Optional[str] = None
) -> None:
    """POST to Meta Cloud API. Silently logs errors.

    token/phone_id default to the global env credentials but should be passed the
    per-tenant values for the receiving WhatsApp number (multi-tenant correctness).
    """
    tok = token or WA_TOKEN
    pid = phone_id or WA_PHONE_ID
    if not tok or not pid:
        logger.warning("WhatsApp credentials not configured; skipping send.")
        return
    url = WA_API_URL.format(phone_id=pid)
    headers = {
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if resp.status_code >= 400:
                logger.error(
                    "WA send FAILED — status=%s phone_id=%s error=%s",
                    resp.status_code, pid, resp.text[:500]
                )
            else:
...
async def wa_incoming(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # Verify Meta's payload signature. When META_APP_SECRET is set, reject any
    # request without a valid X-Hub-Signature-256 (prevents forged webhook calls).
    if settings.META_APP_SECRET:
        ...
        logger.warning("META_APP_SECRET not set — WhatsApp webhook signatures are NOT verified.")
    ...
    try:
        ...
        # Identify the tenant that owns the receiving WhatsApp number, and resolve
        # the per-tenant send credentials (falls back to global env).
        recv_phone_id = (value.get("metadata") or {}).get("phone_number_id")
        school_id_ctx = await _resolve_school_for_phone_id(db, recv_phone_id)
        send_token, send_phone_id = await _wa_send_config(db, school_id_ctx)

        # Delivery-status callbacks (sent / delivered / read / failed) — update MessageLog.
        ...
        if statuses:
            ...
            for st in statuses:
                ...
                if not mid:
                    continue
                vals = {"status": "delivered" if state in ("delivered", "read")
                        else ("failed" if state == "failed" else "sent")}
                if state in ("delivered", "read"):
                    vals["delivered_at"] = datetime.now(timezone.utc)
                await db.execute(_upd(MessageLog).where(MessageLog.provider_message_id == mid).values(**vals))
            await db.commit()
            return {"status": "ok"}
        ...
        # Look up user by phone number (scoped to the receiving tenant when known)
        user = await _lookup_user_by_phone(wa_from, db, school_id=school_id_ctx)
        if not user:
            ...
            ), token=send_token, phone_id=send_phone_id)
            return {"status": "ok"}
        ...
        # Send response messages via Meta API (per-tenant credentials)
        wa_payloads = _serialize_bot_response(wa_from, response)
        for wp in wa_payloads:
            await _send_wa_message(wa_from, wp, token=send_token, phone_id=send_phone_id)
...
class WaSendTestRequest(BaseModel):
    ...
    message: str = "Hello from SeptroSchool! This is a test message."
...
@whatsapp_router.get("/debug-credentials", summary="Check WhatsApp credentials are loaded (super-admin only)")
async def wa_debug_credentials(current_user: User = Depends(get_current_user)):
    """Health check: reports ONLY whether each credential is configured (no secret values).

    Super-admin only — previously this route was unauthenticated and leaked the
    verify token + token prefix.
    """
    ...
    return {
        ...
        "signature_verification": "enforced" if settings.META_APP_SECRET else "DISABLED (set META_APP_SECRET)",
    }
