...
async def list_submissions(
    ...
    # Submissions carry no school_id of their own — scope via the parent homework.
    r = await db.execute(
...
async def create_submission(
    db: AsyncSession, school_id: str, data: HomeworkSubmissionCreate
) -> Optional[HomeworkSubmission]:
    # Only allow submitting against homework that belongs to the caller's school.
    parent = await get_homework(db, school_id, str(data.homework_id))
    if not parent:
        return None
...
async def grade_submission(
    db: AsyncSession, school_id: str, submission_id: str, data: HomeworkSubmissionGrade
) -> Optional[HomeworkSubmission]:
    # Confirm the submission's homework belongs to this school before mutating.
