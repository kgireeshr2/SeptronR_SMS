...
async def create_bulk_message(
    ...
    # Support multiple channels if the schema sent a comma-joined value.
    if channel_val and "," in str(channel_val):
        channels = [c.strip() for c in str(channel_val).split(",") if c.strip()]
    # Schema uses 'audience' but model uses 'target_type'
    audience_val = data_dict.pop("audience", "all")
    # audience_filter may carry {class_id, section_id} for class/section broadcasts.
    audience_filter = data_dict.pop("audience_filter", None) or {}
    class_id = audience_filter.get("class_id") if isinstance(audience_filter, dict) else None
    section_id = audience_filter.get("section_id") if isinstance(audience_filter, dict) else None
    obj = BulkMessage(
        id=uuid.uuid4(),
        school_id=school_id,
