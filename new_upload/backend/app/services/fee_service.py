...
class FeeService:
    ...
    async def collect_fee(self, school_id: str, payload: CollectFeeRequest, collected_by: str) -> FeePaymentResponse:
        ...
        if student_id:
            ...
            notify_event(
                school_id, "fee_receipt", audience="student_parents", student_ids=[student_id],
                default_channels=["in_app", "sms", "whatsapp"],
                title="Fee Payment Received",
                body=(f"Dear Parent, we received a fee payment of Rs.{int(payment.amount) // 100} "
                      f"for {student_name} (Receipt: {payment.receipt_number}). Thank you."),
                context={"amount": int(payment.amount) // 100, "receipt_number": payment.receipt_number,
    ...
    async def init_online_payment(self, school_id: str, payload: OnlinePaymentInitRequest) -> OnlinePaymentInitResponse:
        """Create a real gateway order. Amount is fixed server-side (already validated
        against the invoice balance by the caller); never trusted from a later callback."""
        from app.integrations import payments
        cfg = await payments.resolve_config(self.db, school_id)
        if not cfg["key_id"] or not cfg["key_secret"]:
            raise ValueError("Online payment is not configured for this school")
        try:
            order = await payments.create_order(
                cfg["key_id"], cfg["key_secret"],
                amount_paise=payload.amount,
                receipt=f"inv_{str(payload.invoice_id)[:18]}",
                notes={"invoice_id": str(payload.invoice_id), "school_id": str(school_id)},
            )
        except Exception as ex:
            raise ValueError(f"Payment gateway error: {ex}")
        return OnlinePaymentInitResponse(
            order_id=order.get("id"),
            gateway=payload.gateway,
            amount=int(order.get("amount") or payload.amount),
