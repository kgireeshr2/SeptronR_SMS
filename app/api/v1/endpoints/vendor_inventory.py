...
async def update_vendor(
    ...
    await db.execute(
        ...
        {"id": vendor_id, "sid": school_id, "name": data.name, "cp": data.contact_person,
         ...
         "bank_ifsc": data.bank_ifsc, "active": data.is_active},
    )
    await db.commit()
...
async def _assert_vendor_in_school(db: AsyncSession, vendor_id: str, school_id: str, current_user: User) -> None:
    """Ensure the vendor belongs to the caller's school (super-admins bypass)."""
    if current_user.is_super_admin:
        return
    owner = await _q(
        db, "SELECT id FROM vendors WHERE id=:v AND school_id=:sid",
        {"v": vendor_id, "sid": str(school_id)},
    )
    if not owner:
        raise HTTPException(status_code=404, detail="Vendor not found")
...
async def list_products(
    ...
    await _assert_vendor_in_school(db, vendor_id, school_id, current_user)
    rows = await _q(
...
async def update_product(
    ...
    if not current_user.is_super_admin:
        owner = await _q(
            db,
            "SELECT vp.id FROM vendor_products vp JOIN vendors v ON v.id = vp.vendor_id "
            "WHERE vp.id=:id AND v.school_id=:sid",
            {"id": product_id, "sid": str(school_id)},
        )
        if not owner:
...
async def get_invoice_items(
    ...
    if not current_user.is_super_admin:
        owner = await _q(
            db, "SELECT id FROM vendor_invoices WHERE id=:id AND school_id=:sid",
            {"id": invoice_id, "sid": str(school_id)},
        )
        if not owner:
            raise HTTPException(status_code=404, detail="Invoice not found")
    rows = await _q(
...
async def get_sale_items(
    ...
    if not current_user.is_super_admin:
        owner = await _q(
            db, "SELECT id FROM vendor_sales WHERE id=:id AND school_id=:sid",
            {"id": sale_id, "sid": str(school_id)},
        )
        if not owner:
