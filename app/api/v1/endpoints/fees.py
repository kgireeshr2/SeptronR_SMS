...
async def get_student_statement(
    ...
    academic_year_id: Optional[str] = Query(None),
    school_id: str = Depends(get_school_id),
    # `fees:view` so parents/students (who hold view, not report) can see a
    # statement for their own/child's record — same role-based posture as the
    # attendance/exam/timetable self-service reads.
    _: User = Depends(permission_required("fees", "view")),
    db: AsyncSession = Depends(get_db),
):
    # Default to the current academic year when the caller doesn't pass one
    # (mobile self-service doesn't know the year id).
    if not academic_year_id:
        ...
        academic_year_id = str(current.id)
...
async def init_online_payment(
    ...
    service = FeeService(db)
    try:
        return await service.init_online_payment(school_id, payload)
    except ValueError as ex:
        raise HTTPException(status_code=400, detail=str(ex))
...
async def online_payment_callback(
    ...
    """Confirm a Razorpay checkout. The callback signature is verified and the paid
    amount is reconciled FROM THE GATEWAY — the client-supplied amount is ignored."""
    from app.integrations import payments

    cfg = await payments.resolve_config(db, school_id)
    if not cfg["key_secret"]:
        raise HTTPException(status_code=400, detail="Online payment is not configured for this school")
    if not payments.verify_payment_signature(
        payload.order_id,
        payload.razorpay_payment_id or "",
        payload.razorpay_signature or "",
        cfg["key_secret"],
    ):
        raise HTTPException(status_code=400, detail="Payment signature verification failed")

    # Reconcile the authoritative amount from the gateway order.
    try:
        order = await payments.fetch_order(cfg["key_id"], cfg["key_secret"], payload.order_id)
    except Exception as ex:
        raise HTTPException(status_code=502, detail=f"Could not verify payment with gateway: {ex}")
    amount = int(order.get("amount_paid") or order.get("amount") or 0)
    if amount <= 0:
        raise HTTPException(status_code=400, detail="Gateway reports no captured amount for this order")
    ...
    try:
        return await service.collect_fee(
            ...
            CollectFeeRequest(
                ...
                remarks=f"razorpay_order={payload.order_id}",
            ),
            str(system_user.id),
        )
    except ValueError as ex:
