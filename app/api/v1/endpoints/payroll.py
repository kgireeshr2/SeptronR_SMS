...
async def mark_payroll_as_paid(
    ...
    # Notify each paid staff member (in-app + email).
    try:
        from sqlalchemy import select as _select
        from app.models.staff import Staff as _Staff, StaffPayroll as _Payroll
        from app.services.notifications.notify import notify_event
        rows = (await db.execute(
            _select(_Staff.user_id)
            .join(_Payroll, _Payroll.staff_id == _Staff.id)
            .where(_Payroll.id.in_(mark_paid_data.payroll_ids))
        )).scalars().all()
        uids = [str(u) for u in rows if u]
        if uids:
            notify_event(
                school_id, "payslip", audience="custom", user_ids=uids,
                default_channels=["in_app", "email"],
                title="Salary Paid",
                body="Your salary has been processed and marked as paid. "
