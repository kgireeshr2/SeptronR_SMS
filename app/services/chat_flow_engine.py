...
async def get_user_role(db: AsyncSession, user: User) -> str:
    ...
    for keyword in ["teacher", "faculty", "educator", "instructor"]:
        if any(keyword in n for n in names):
            return "teacher"
    for keyword in ["accountant", "account", "cashier", "finance", "bursar"]:
        if any(keyword in n for n in names):
            return "accountant"
...
# Action synonyms — the seed uses 'edit' but expanded 'manage' grants emit 'update',
# and the web layer aliases read↔view. Keep the bot's permission check in sync so a
# gate written as ("students", "update") still matches a seeded "students:edit" grant.
_ACTION_ALIASES: Dict[str, set] = {
    "edit": {"edit", "update"},
    "update": {"edit", "update"},
    "view": {"view", "read"},
    "read": {"view", "read"},
...
def _has_perm(permissions: set, module: str, action: str) -> bool:
    """Check if user has a specific permission (alias- and manage-aware)."""
    if "*:*" in permissions or f"{module}:manage" in permissions:
        return True
    for act in _ACTION_ALIASES.get(action, {action}):
        if f"{module}:{act}" in permissions:
            return True
    return False


# ─── Flow → required permission map ───────────────────────────────────────────
# None  → available to any authenticated user (self-service / personal).
# "__super__" → super-admin only.
# (module, action) → checked against the user's resolved permissions.
FLOW_PERMISSIONS: Dict[str, Any] = {
    "greeting":           None,
    "attendance_today":   ("attendance", "view"),
    "student_attendance": ("attendance", "create"),
    "staff_attendance":   ("staff", "edit"),
    "attendance_view":    ("attendance", "view"),
    "fee":                ("fees", "view"),
    "accounting":         ("accounting", "view"),
    "leave":              None,
    "marks":              ("exams", "view"),
    "marks_entry":        ("exams", "edit"),
    "student_details":    ("students", "view"),
    "student_edit":       ("students", "edit"),
    "sa_schools":         "__super__",
    "sa_plans":           "__super__",
}


def _option_permitted(flow: str, permissions: set, user: User) -> bool:
    """Pure permission decision for a menu option (no DB). Used to filter menus."""
    req = FLOW_PERMISSIONS.get(flow, None)
    if req is None:
        return True
    if req == "__super__":
        return bool(getattr(user, "is_super_admin", False))
    if getattr(user, "is_super_admin", False) or "*:*" in permissions:
        return True
    module, action = req
    return _has_perm(permissions, module, action)


def _parse_marks_tokens(
    message: str, roster_len: int, full_marks: float
) -> Tuple[List[Tuple[int, Optional[float], bool]], List[str]]:
    """Parse a marks-entry reply like '1:45, 2:50, 3:ab' into structured updates.

    Returns (updates, errors) where each update is (index, marks|None, is_absent).
    Index is 1-based as shown to the user; validation against roster bounds and the
    exam's full_marks happens here so the save step stays simple.
    """
    updates: List[Tuple[int, Optional[float], bool]] = []
    errors: List[str] = []
    for raw in re.split(r"[,\n;]+", message.strip()):
        token = raw.strip()
        if not token:
            continue
        m = re.match(r"^(\d+)\s*[:=\-\s]\s*(.+)$", token)
        if not m:
            errors.append(token)
            continue
        idx = int(m.group(1))
        val = m.group(2).strip().lower()
        if idx < 1 or idx > roster_len:
            errors.append(f"{token} (no #{idx})")
            continue
        if val in ("ab", "absent", "a"):
            updates.append((idx, None, True))
            continue
        try:
            marks = float(val)
        except ValueError:
        ...
        if marks < 0 or marks > full_marks:
            errors.append(f"{token} (0-{_fmtnum(full_marks)})")
            continue
        updates.append((idx, marks, False))
    return updates, errors


def _fmtnum(v: Any) -> str:
    """Format a number dropping trailing zeros (45.0 → '45', 45.5 → '45.5')."""
    try:
        f = float(v)
...
MENUS: Dict[str, List[Dict[str, str]]] = {
    ...
    "admin": [
        ...
        {"id": "marks_entry",        "label": "Enter Marks",          "description": "Enter / update exam results"},
        {"id": "student_details",    "label": "Student Info",         "description": "Search student details"},
        {"id": "student_edit",       "label": "Edit Student",         "description": "Search and edit student info"},
    ],
    "teacher": [
        ...
        {"id": "marks_entry",        "label": "Enter Marks",          "description": "Enter / update exam results"},
        {"id": "student_edit",       "label": "Edit Student",         "description": "Update student info"},
...
class FlowEngine:
    ...
    async def handle(
        ...
        if not session.current_flow:
            ...
            _all_flow_ids = {
                ...
                "attendance_view", "fee", "accounting", "leave", "marks", "marks_entry",
    ...
    async def _route_flow(
        ...
        handlers = {
            ...
            "marks_entry":        self._flow_marks_entry,
        ...
        if not handler:
            session.current_flow = None
            return await self._start_greeting(session, user, db)
        # Authoritative permission gate — enforced here (not just in the menu) so a
        # user cannot reach a flow by typing its ID or replaying a stale action button.
        if not await self._flow_permitted(flow, user, db):
            session.current_flow = None
            session.current_step = None
            session.set_context({})
            return BotResponse(
                text="⛔ You don't have permission to use this option.\n\n"
                     "Type *menu* to see the actions available to you.",
                type="error",
                session_ended=True,
            )
        try:
    ...
    async def _start_greeting(
        ...
        session.current_step = None
        session.set_context({})
        options = await self._visible_menu(db, user)
        name = (user.first_name or user.username or "there")
        if not options:
            return BotResponse(
                text=(
                    f"👋 Hi **{name}**! Your account doesn't have access to any "
                    f"self-service actions yet. Please contact your school administrator."
                ),
                type="info",
                session_ended=True,
            )
    ...
    async def _visible_menu(self, db: AsyncSession, user: User) -> List[Dict[str, str]]:
        """Return the role's menu filtered to options the user is actually permitted to use."""
        role = await get_user_role(db, user)
        base = MENUS.get(role, MENUS["staff"])
        if user.is_super_admin:
            return base
        perms = await get_user_permissions(db, user)
        visible: List[Dict[str, str]] = []
        for opt in base:
            if _option_permitted(opt["id"], perms, user):
                visible.append(opt)
        return visible

    async def _flow_permitted(self, flow: str, user: User, db: AsyncSession) -> bool:
        """Authoritative permission check for entering a flow (used by the router)."""
        req = FLOW_PERMISSIONS.get(flow, None)
        if req is None:
            return True
        if req == "__super__":
        ...
        perms = await get_user_permissions(db, user)
        module, action = req
        return _has_perm(perms, module, action)
    ...
    async def _sta_load_staff(self, session, db, school_id) -> BotResponse:
        ...
        staff = await _q(
            ...
            "COALESCE(department_id::text, '') AS dept_id "
    ...
    async def _sta_save(self, session, payload, user, db, school_id) -> BotResponse:
        ...
        for sid in present_ids:
            # Insert-if-absent (portable on Postgres): only create when today's row is missing.
            await db.execute(
                text(
                    "INSERT INTO staff_attendance "
                    "(id, school_id, staff_id, date, status, source, created_at, updated_at) "
                    "SELECT :id, :school_id, :sid, CAST(NOW() AS DATE), 'present', 'manual', NOW(), NOW() "
                    "WHERE NOT EXISTS (SELECT 1 FROM staff_attendance "
                    "                  WHERE staff_id = :sid AND date = CAST(NOW() AS DATE))"
                ),
        ...
        for sid in absent_ids:
            await db.execute(
                text(
                    "INSERT INTO staff_attendance "
                    "(id, school_id, staff_id, date, status, source, created_at, updated_at) "
                    "SELECT :id, :school_id, :sid, CAST(NOW() AS DATE), 'absent', 'manual', NOW(), NOW() "
                    "WHERE NOT EXISTS (SELECT 1 FROM staff_attendance "
                    "                  WHERE staff_id = :sid AND date = CAST(NOW() AS DATE))"
                ),
                {"id": _new_id(), "school_id": school_id, "sid": sid},
    ...
    async def _fee_dispatch(self, session, message, user, db, school_id) -> BotResponse:
        ...
        if message in ("fee_collect", ) or "collect" in msg or "pay" in msg:
            perms = await get_user_permissions(db, user)
            if not _has_perm(perms, "fees", "collect"):
                session.current_step = "fee_action"
                return BotResponse(
                    text="⛔ You don't have permission to collect fee payments.\n\n"
    ...
    async def _fee_defaulters(self, session, db, school_id) -> BotResponse:
        ...
        rows = await _q(
            ...
            f"""SELECT s.first_name || ' ' || s.last_name AS student_name,
               ...
               WHERE {s_where if s_where else '1=1 AND'} fi.status NOT IN ('paid','cancelled')
    ...
    async def _leave_save(self, session, user, db, school_id) -> BotResponse:
        ...
        await db.execute(
            text(
                ...
                "(id, school_id, staff_id, leave_type_id, from_date, to_date, days_count, "
    ...
    # ─── MARKS ENTRY (Exam Results Update) ────────────────────────────────────

    async def _flow_marks_entry(
        self, session, message, payload, user, db, school_id
    ) -> BotResponse:
        step = session.current_step or "select_exam"
        if step == "select_exam":
            return await self._me_list_exams(session, db, school_id)
        if step == "load_roster":
            return await self._me_load_roster(session, message, db, school_id)
        if step == "save_marks":
            return await self._me_save(session, message, user, db, school_id)
        return await self._me_list_exams(session, db, school_id)

    async def _me_list_exams(self, session, db, school_id) -> BotResponse:
        exams = await _q(
            db,
            "SELECT e.id, e.name, e.full_marks, COALESCE(sub.name,'') AS subject "
            "FROM exams e LEFT JOIN subjects sub ON e.subject_id = sub.id "
            "WHERE e.school_id = :sid ORDER BY e.exam_date DESC NULLS LAST LIMIT 10",
            {"sid": school_id},
        )
        if not exams:
            session.current_flow = None
            return BotResponse(
                text="No exams found. Create an exam in the web app first.",
                type="info", session_ended=True,
            )
        session.current_step = "load_roster"
        session.set_context({})
        options = [
            BotOption(
                id=str(e["id"]),
                label=str(e["name"])[:24],
                description=f"{e['subject']} | out of {int(e['full_marks'] or 100)}",
            )
            for e in exams
        ]
        return BotResponse(
            text="📝 **Enter Exam Marks**\n\nSelect an exam:",
            type="options",
            options=options,
            breadcrumb="Marks Entry",
        )

    async def _me_load_roster(self, session, message, db, school_id) -> BotResponse:
        ctx = session.context
        if "me_exam_id" not in ctx:
            exams = await _q(
                db,
                "SELECT id, name, full_marks, pass_marks, class_id, section_id "
                "FROM exams WHERE school_id = :sid",
                {"sid": school_id},
            )
            matched = next((e for e in exams if str(e["id"]) == message), None)
            if not matched:
                matched = next(
                    (e for e in exams if message.lower() in str(e["name"]).lower()), None
                )
            if not matched:
                session.current_flow = None
                return BotResponse(
                    text="Exam not found. Type **menu** to start over.",
                    type="error", session_ended=True,
                )
            session.patch_context({
                "me_exam_id":   str(matched["id"]),
                "me_exam_name": matched["name"],
                "me_full":      float(matched["full_marks"] or 100),
                "me_class_id":  str(matched["class_id"]),
                "me_section_id": str(matched["section_id"]) if matched["section_id"] else "",
            })
            ctx = session.context

        params: Dict[str, Any] = {
            "sid": school_id, "cid": ctx["me_class_id"], "eid": ctx["me_exam_id"],
        }
        sec_clause = ""
        if ctx.get("me_section_id"):
            sec_clause = "AND se.section_id = :sec"
            params["sec"] = ctx["me_section_id"]

        students = await _q(
            db,
            f"""SELECT s.id, s.first_name || ' ' || s.last_name AS name,
                 COALESCE(se.roll_number, '') AS roll,
                 sm.marks_obtained, sm.is_absent
               FROM students s
               JOIN student_enrollments se ON se.student_id = s.id
                  AND se.class_id = :cid AND se.is_current = true AND se.school_id = :sid {sec_clause}
               LEFT JOIN student_marks sm ON sm.student_id = s.id AND sm.exam_id = :eid
               WHERE s.school_id = :sid AND s.is_active = true
               ORDER BY se.roll_number, s.first_name""",
            params,
        )
        if not students:
            session.current_flow = None
            return BotResponse(
                text="No students found for this exam's class/section.",
                type="error", session_ended=True,
            )

        roster = [
            {"student_id": str(s["id"]), "name": s["name"], "roll": str(s["roll"] or "")}
            for s in students
        ]
        session.patch_context({"me_roster": roster, "me_saved_count": 0})
        session.current_step = "save_marks"

        full = int(ctx["me_full"])
        lines = [f"📝 **{ctx['me_exam_name']}** — enter marks (out of {full}).", ""]
        for i, s in enumerate(students, 1):
            if s["is_absent"]:
                cur = " — [AB]"
            elif s["marks_obtained"] is not None:
                cur = f" — [{_fmtnum(s['marks_obtained'])}]"
            else:
                cur = ""
            lines.append(f"{i}. {s['name']} ({s['roll'] or '—'}){cur}")
        lines.append("")
        lines.append("Reply like: **1:45, 2:50, 3:ab** (use *ab* for absent).")
        lines.append("Send in batches if you like. Type **done** when finished.")
        return BotResponse(
            text="\n".join(lines),
            type="question",
            breadcrumb=f"Marks Entry › {ctx['me_exam_name']}",
        )

    async def _me_save(self, session, message, user, db, school_id) -> BotResponse:
        ctx = session.context
        msg = message.strip().lower()

        if msg in ("done", "finish", "complete"):
            total = ctx.get("me_saved_count", 0)
            exam_name = ctx.get("me_exam_name", "")
            session.current_flow = None
            session.current_step = None
            session.set_context({})
            return BotResponse(
                text=(
                    f"✅ **Marks Entry Complete**\n\n"
                    f"Exam: {exam_name}\nSaved: **{total}** record(s)\n\n"
                    f"Type **menu** to continue."
                ),
                type="success",
                actions=[{"label": "Exam Results", "path": "/exams"}],
                session_ended=True,
            )

        roster = ctx.get("me_roster", [])
        exam_id = ctx.get("me_exam_id")
        full = float(ctx.get("me_full", 100) or 100)
        if not roster or not exam_id:
            session.current_flow = None
            return BotResponse(
                text="⚠️ Session expired. Type **menu** to restart.",
                type="error", session_ended=True,
            )

        updates, errors = _parse_marks_tokens(message, len(roster), full)
        if not updates:
            hint = f"\nCouldn't read: {', '.join(errors[:5])}" if errors else ""
            return BotResponse(
                text=f"Please use the format **1:45, 2:ab**.{hint}",
                type="question",
                breadcrumb=f"Marks Entry › {ctx.get('me_exam_name', '')}",
            )

        saved = 0
        for idx, marks, is_absent in updates:
            stu = roster[idx - 1]
            grade = None
            if not is_absent and marks is not None and full:
                grade = await self._grade_for(db, school_id, round(marks / full * 100, 2))
            await db.execute(
                text(
                    "INSERT INTO student_marks "
                    "(id, school_id, exam_id, student_id, marks_obtained, is_absent, grade, "
                    " entered_by, updated_by, created_at, updated_at) "
                    "VALUES (:id, :sid, :eid, :stu, :marks, :absent, :grade, :uid, :uid, NOW(), NOW()) "
                    "ON CONFLICT (exam_id, student_id) DO UPDATE SET "
                    "  marks_obtained = EXCLUDED.marks_obtained, is_absent = EXCLUDED.is_absent, "
                    "  grade = EXCLUDED.grade, updated_by = :uid, updated_at = NOW()"
                ),
                {
                    "id": _new_id(), "sid": school_id, "eid": exam_id,
                    "stu": stu["student_id"], "marks": marks, "absent": is_absent,
                    "grade": grade, "uid": str(user.id),
                },
            )
            saved += 1
        await db.commit()

        total = ctx.get("me_saved_count", 0) + saved
        session.patch_context({"me_saved_count": total})

        detail = ", ".join(
            f"#{idx}={'AB' if absent else _fmtnum(mk)}" for idx, mk, absent in updates[:8]
        )
        note = f"\n⚠️ Skipped: {', '.join(errors[:5])}" if errors else ""
        return BotResponse(
            text=(
                f"✅ Saved {saved} mark(s) ({detail}). Running total: **{total}**.{note}\n\n"
                f"Send more (e.g. **5:38**) or type **done** to finish."
            ),
            type="question",
            breadcrumb=f"Marks Entry › {ctx.get('me_exam_name', '')}",
        )

    async def _grade_for(self, db, school_id, pct: float) -> Optional[str]:
        """Resolve a letter grade from the school's grading scale (None if unset)."""
        rows = await _q(
            db,
            "SELECT ranges FROM grading_scales WHERE school_id = :sid AND is_active = true",
            {"sid": school_id},
        )
        if not rows:
            return None
        ...
        for r in ranges or []:
            try:
                lo = float(r.get("min_pct", r.get("min", 0)))
                hi = float(r.get("max_pct", r.get("max", 100)))
            except (TypeError, ValueError):
                continue
            if lo <= pct <= hi:
                return r.get("grade")
    ...
    async def _se_confirm(self, session, message, db, school_id) -> BotResponse:
        ...
        # is_active is a boolean column — bind a real bool (asyncpg rejects 1/0 for bool).
        sql_val = (new_value == "true") if field == "status" else new_value
