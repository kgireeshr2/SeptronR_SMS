...
class SchoolRepository:
    ...
    async def get_by_hostname(self, hostname: str) -> Optional[School]:
        """Resolve a school from a verified custom domain hostname."""
        from app.models.website import SchoolDomain

        result = await self.db.execute(
            select(School)
            .join(SchoolDomain, SchoolDomain.school_id == School.id)
            .where(func.lower(SchoolDomain.hostname) == hostname.lower())
            .where(SchoolDomain.is_verified.is_(True))
        )
        return result.scalar_one_or_none()
