...
async def get_student_ledger(
    ...
    repo = FeeV2Repository(db)
    ledger = await repo.get_student_ledger(school_id, student_id, academic_year_id)
    if not ledger:
        raise HTTPException(status_code=404, detail="No fee assignment found for this student in the given year")
    return ledger
