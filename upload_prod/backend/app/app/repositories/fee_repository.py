...
class FeeRepository:
    ...
    async def get_invoice(self, school_id: str, invoice_id: str) -> FeeInvoice | None:
        result = await self.db.execute(
            select(FeeInvoice).where(FeeInvoice.school_id == school_id, FeeInvoice.id == invoice_id)
        )
        return result.scalar_one_or_none()
    ...
    async def collect_payment(
        ...
        invoice = await self.get_invoice(school_id, invoice_id)
        if not invoice:
