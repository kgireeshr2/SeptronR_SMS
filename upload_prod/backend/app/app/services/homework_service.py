...
async def create_homework(db: AsyncSession, school_id: str, teacher_id: UUID, data: HomeworkCreate):
    return await repo.create_homework(db, school_id, teacher_id, data)
...
async def submit_homework(db: AsyncSession, data: HomeworkSubmissionCreate):
    return await repo.create_submission(db, data)


async def grade_submission(db: AsyncSession, submission_id: str, data: HomeworkSubmissionGrade):
    return await repo.grade_submission(db, submission_id, data)
