...
async def get_section_timetable(section_id: str, academic_year_id: str = Query(...), db: AsyncSession = Depends(get_db)):
    grid = await TimetableRepository(db).get_grid_for_section(section_id, academic_year_id)
...
async def delete_slot(timetable_id: str, db: AsyncSession = Depends(get_db)):
    await TimetableRepository(db).delete_entry(timetable_id)
...
async def get_teacher_schedule(teacher_id: str, academic_year_id: str = Query(...), db: AsyncSession = Depends(get_db)):
    schedule = await TimetableRepository(db).get_teacher_schedule(teacher_id, academic_year_id)
...
async def export_section_timetable(section_id: str, academic_year_id: str = Query(...), db: AsyncSession = Depends(get_db)):
    service = TimetableService(TimetableRepository(db), SectionRepository(db), SubjectRepository(db))
    pdf_data = await service.get_timetable_pdf(section_id, academic_year_id)
    return Response(
        content=pdf_data,
